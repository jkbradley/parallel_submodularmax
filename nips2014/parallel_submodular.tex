...
A transaction obtains from the server a set of conditions $\mathfrak{B}_e$ and an associated logical time $i$, such that $\mathfrak{B}_e$ is guaranteed to hold at $i$.
...
It is straightforward to see that the output of the parallel execution is $T^\partial_n (T^\partial_{n-1}(\ldots T^\partial_1(\emptyset, V)))$.
...
This is achieved by operating on potentially stale states -- the guarantee reduces to requiring $\mathfrak{B}_e$ be a stale version of $\mathfrak{S}$, and logical ordering is implicitly defined by the time of commit.
  ...
  \begin{multicols}{2}
      ...
      \begin{algorithm}[H]
        ...
        \ParForAll{$p \in \set{1, \ldots, P}$}{
          \While{$\exists$ element to process}{
            ...
            $T^\partial_i$ = propose($e$, $\mathfrak{B}_e$)\;
            commit($e$, $i$, $T^\partial_i$) \tcp{Non-blocking}
          }
        }
      \end{algorithm}
    ...
    \begin{minipage}{0.45\textwidth}
      \begin{algorithm}[H]
        ...
        \Atomically{
        \If{$T^\partial_i =\text{Fail}$}{
          \tcp{Reject and compensate}
          $T^\partial_i$ = propose($e$, $\mathfrak{S}$)\;
        }
        \tcp{Advance program state}
        $\mathfrak{S} \leftarrow T^\partial_i(\mathfrak{S})$\;
        }
      \end{algorithm}
    \end{minipage}
