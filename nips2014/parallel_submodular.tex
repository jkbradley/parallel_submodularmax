...
\item We prove the serial equivalence of \occ{} to \seqalg{}; we analytically bound the amount of coordination of \occ{} for two example problems.
...
The double greedy algorithm \cite{buchbinder2012} gives an approximation of $E[F(A)] = 1/2 f(OPT)$, where $A$ is the algorithm output, and $OPT$ is an optimal solution.

\hogwild{} can give an approximation of $E[F(A)] = \frac{1}{2} F(OPT) - \frac{1}{4}\sum_iE[\rho_i]$, where $\rho_i$ is the maximum difference in the marginal gain that may result from not knowing the full information when deciding whether to include or exclude element $i$.

\occ{} guarantees an outcome that is equivalent to a sequential run of \seqalg{}.
Theoretical properties of \seqalg{} immediately translates to \occ{} -- in particular, \occ{} gives the same approximation factor of $1/2$.
...
\section{\hogwild{} Double Greedy Algorithm}
\xinghao{======== Provide more intuition for what \hogwild{} is doing.}
...
\hogwild{} closely resembles the serial \seqalg{}, but the elements $e \in V$ are no longer processed in a fixed order.  Thus, the sets $A, B$ are replaced by ``bounds'' $\hat{A}, \hat{B}$, where $\hat{A}$ is a subset of the ``true'' $A$ and $\hat{B}$ is a superset of the ``actual'' $B$ on each iteration.
...
We order the elements $e \in V$ according to the time at which \algref{alg:hogwild} line \ref{alg:hogwild:time} is executed.
...
\section{Concurrency Control for Double Greedy Algorithm}
