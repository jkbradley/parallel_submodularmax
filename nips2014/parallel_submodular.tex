...
The second, \emph{concurrency control} approach guarantees the tight 1/2-approximation, at the quantifiable cost of additional coordination and reduced parallelism.
...
For example, graphical model inference \cite{gillenwater12,reed13} and feature selection with a modular cost function \joey{Is there a better example or at least a citation?} (\eg functions of the form $F(S) = G(S) - \lambda M(S)$, where $G(S)$ is monotone submodular and $M(S)$ is a linear (modular) cost function) require maximizing non-monotone submodular functions.
...
For non-monotone functions, the simple greedy algorithm in \cite{nemhauser1978} can perform arbitrarily poorly (see the Appendix for an example).
Intuitively, the introduction of additional elements to monotone submodular functions never decreases the objective while introducing elements to non-monotone submodular functions can \emph{decrease} the objective to its minimum.
...
In iteration $i$, the set $A^i$ contains the items selected before item/iteration $i$, and $B^i$ contains $A^i$ and the items that are so far undecided.
The algorithm sequentially passes through the items in $V$ and determines online whether to keep item $i$ (add to $A^i$) or discard it (remove from $B^i$).
...
In \figref{fig:transactionmodel} we describe a bounded optimistic pattern for concurrency control.
