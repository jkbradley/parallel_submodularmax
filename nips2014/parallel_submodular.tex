  ...
  \begin{multicols}{2}
      ...
      \begin{algorithm}[H]
        ...
        \ParForAll{$p \in \set{1, \ldots, P}$}{
          \While{$\exists$ element to process}{
            ...
            (op, $\mathfrak{A}_e$) = propose($e$, $\mathfrak{S}_e$)\;
            validate($e$, op, $\mathfrak{A}_e$)
          }
        }
      \end{algorithm}
    ...
    \begin{minipage}{0.49\textwidth}
      \begin{algorithm}[H]
        ...
        \If{op = fail $\vee \neg \mathfrak{A}_e(\mathfrak{S})$}{
          \CommentSty{// Reject, and re-compute}\;
          op = repropose($e$, $\mathfrak{S}$)\;
        }
        \CommentSty{// Apply op to current state}\;
        $\mathfrak{S} \leftarrow \text{op}(\mathfrak{S})$\;
      \end{algorithm}
    \end{minipage}
