...
\begin{figure}[h]
  ...
  \begin{multicols}{2}
    \begin{minipage}{0.49\textwidth}
      ...
      \begin{algorithm}[H]
        \DontPrintSemicolon
        \caption{Parallel bidirectional greedy}
        \label{alg:par}
        \lFor{$i\in V$}{$\hat{A}(i) = \tilde{A}(i) = 0$, $\hat{B}(i) \tilde{B}(i) = 1$}
        \ParForAll{$p \in \set{1, \ldots, P}$}{
          \While{$\exists$ element to process}{
            $i = $ next element to process\;
            $(\hat{A}_i, \tilde{A}_i, \hat{B}_i, \tilde{B}_i)$ = getSnapshot($i$)\;
            Draw $u_i\sim Unif(0,1)$\;
            (res($i$), $\hat{A}_i$, $\tilde{A}_i$, $\hat{B}_i$, $\tilde{B}_i$) = $\Gamma_i(\hat{A}_i, \tilde{A}_i, \hat{B}_i, \tilde{B}_i, u_i)$\;
            validate($i$, res($i$), $u_i$, $\hat{A}_i$, $\tilde{A}_i$, $\hat{B}_i$, $\tilde{B}_i$)\;
          }
        }
      \end{algorithm}
    ...
    \begin{minipage}{0.49\textwidth}
      ...
      \begin{algorithm}[H]
        \DontPrintSemicolon
        \caption{Parallel getSnapshot($i$)}
        \label{alg:parsnapshot}
        $\tilde{A}(i) \leftarrow 1$; $\tilde{B}(i) \leftarrow 0$\;
        \Return ($\hat{A}$, $\tilde{A}$, $\hat{B}$, $\tilde{B}$)
      \end{algorithm}

      \begin{algorithm}[H]
        \DontPrintSemicolon
        \caption{Parallel validate}
        \label{alg:parvalidate}
        \If{$\text{res}(i) = 0 \vee \hat{A}_i \not\subseteq A^{i-1} \not\subseteq \tilde{A}_i \vee \hat{B}_e \not\supseteq B^{i-1} \not\supseteq \tilde{B}_i$}{
          $\Delta_+(i) = F(A^{i-1} \cup i) - F(A^{i-1})$\;
          $\Delta_-(i) = F(B^{i-1} \backslash i) - F(B^{i-1})$\;
          \lIf {$u_e < \frac{[\Delta_+(i)]_+}{[\Delta_+(i)]_+ + [\Delta_-(i)]_+}$}{
            res$(i) \leftarrow 1$
          }\lElse{
            res$(i) \leftarrow -1$
          }
        ...
        \If {res$(i) = 1$}{
          ...
          $B^i := B^{i-1}$\;
          $\hat{A}(i)   \leftarrow 1$;
          $\tilde{B}(i) \leftarrow 1$\;
        }\Else{
          $A^i := A^{i-1}$;
          $B^i := B^{i-1}\backslash i$\;
          $\tilde{A}(i) \leftarrow 0$;
          $\hat{B}(i)   \leftarrow 0$\;
        }
      \end{algorithm}
      ...
      \begin{algorithm}[H]
        ...
        \ParForAll{$p \in \set{1, \ldots, P}$}{
          \While{$\exists$ element to process}{
            ...
            $\Delta_+^{\min}(e) = F(\tilde{A}) - F(\tilde{A} \cup e)$\;\label{alg:occ:deltaplusmin}
