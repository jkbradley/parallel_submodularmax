...
Based on the snapshop and selected element, each processor proposes a transaction, $T^\partial_e$, and the preconditions under which the transaction is valid, $\mathfrak{A}$.
We denote the transaction as $T^\partial_e$ to avoid confusion with \eqnref{eqn:greedytransaction} and because it encodes the effect (\eg remove $e$ from $B$) of more costly transactions (\eg \eqnref{eqn:greedytransaction}).
...
\begin{figure}[h]
  ...
  \begin{multicols}{2}
      ...
      \begin{algorithm}[H]
        ...
        \ParForAll{$p \in \set{1, \ldots, P}$}{
          \While{$\exists$ element to process}{
            ...
            $\mathfrak{S}_e = \text{getSnapshot}$\;
            ($T^\partial_e$, $\mathfrak{A}_e$) = propose($e$, $\mathfrak{S}_e$)\;
            validate($e$, $T^\partial_e$, $\mathfrak{A}_e$)
          }
        }
      \end{algorithm}
    ...
    \begin{minipage}{0.45\textwidth}
      \begin{algorithm}[H]
        ...
        \DontPrintSemicolon
        \caption{Validate}
        \label{alg:generalparallel:validate}
        \Atomically{
        \If{$T^\partial_e =\text{Fail}$ or $\mathfrak{S}$ does not satisfy $\mathfrak{A}_e$}{
          \tcp{Compensating Action}
          $T^\partial_e$ = repropose($e$, $\mathfrak{S}$)\;
        }
        \tcp{Advance program state}
        $\mathfrak{S} \leftarrow T^\partial_e(\mathfrak{S})$\;
        }
      \end{algorithm}
    \end{minipage}
  ...
  \end{multicols}
  \caption{\footnotesize Algorithm for generalized transactions. Each transaction makes a copy $\mathfrak{S}_e$ of the current state $\mathfrak{S}$, and proposes an operation op and a set of associated assumptions $\mathfrak{A}_e$. The valdiation procedure verifies that the assumptions hold on the current state, and ultimately applies op to the $\mathfrak{S}$.}
  \label{fig:submax}
\end{figure}
