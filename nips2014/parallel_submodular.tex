...
In \occ{}, during the commit process for deferred element $e$, we have $\hat{A} = A^{\iota(e)-1}$ and $\hat{B} = B^{\iota(e)-1}$. \joey{what is a deferred element?}
\end{restatable}
...
\joey{Since we don't seem to use the lemma perhaps we can move it to the proof and state the corollary as the key lemma?}

\begin{cor}\label{cor:occ:delta_bound} Submodularity of $F$ implies that the $\Delta$'s computed by \occ{} satisfy $\Delta_+^{\min}(e) \leq \Delta_+(e) \leq \Delta_+^{\max}(e)$ and $\Delta_-^{\min}(e) \leq \Delta_-(e) \leq \Delta_-^{\max}(e)$.
% \begin{align}
...
\joey{What does this all mean?  Interpret the consequences of these results.}
