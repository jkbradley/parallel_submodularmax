...
\begin{figure}[h]
  ...
  \begin{multicols}{2}
      ...
      \begin{algorithm}[H]
        ...
        \ParForAll{$p \in \set{1, \ldots, P}$}{
          \While{$\exists$ element to process}{
            ...
            $(\mathfrak{B}_e, i) = \text{getBounds}(e)$\;
            $T^\partial_e$ = propose($e$, $\mathfrak{B}_e$)\;
            commit($e$, $i$, $T^\partial_e$) \tcp{Non-blocking}
          }
        }
      \end{algorithm}
    ...
    \begin{minipage}{0.45\textwidth}
      \begin{algorithm}[H]
        ...
        \Atomically{
        \If{$T^\partial_e =\text{Fail}$}{
          \tcp{Reject and compensate}
          $T^\partial_e$ = propose($e$, $\mathfrak{S}$)\;
        }
  ...
  \end{multicols}
  \caption{\footnotesize Algorithm for generalized transactions. Each transaction requests for its position $i$ in the commit ordering, as well as the bounds $\mathfrak{B}_e$ that are guaranteed to hold when it commits. Transactions are also guaranteed to be committed according to the given ordering.}
  \label{fig:submax}
\end{figure}
