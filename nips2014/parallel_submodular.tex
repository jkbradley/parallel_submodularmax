...
The \occ{} algorithm \xinghao{for the lack of a better name} guarantees an outcome that is equivalent to a sequential run of the double greedy algorithm.
Theoretical properties of the double greedy algorithm immediately translates to the \occ{} algorithm -- in particular, the \occ{} algorithm gives the same approximation factor of $1/2$.
...
These ``true'' sets $A^i, B^i$ provide a serialization against which we can compare the \hogwild{} algorithm; in this serialization, \algref{alg:submax} computes:
\begin{align*}
...
We define $A^{\iota(e)-1}$, $B^{\iota(e)-1}$ as before with the \hogwild{} algorithm.
...
The \hogwild{} algorithm solves the unconstrained problem $\max_{A\subset V} F(A)$ with approximation
$
...
\begin{thm} \occ{} is serially equivalent to double greedy.
\end{thm}
\begin{proof}
...
\begin{figure}[ht]
  ...
  Fig \ref{fig:difffa_maxgraphcut}, \ref{fig:difffa_setcover} -- \% difference between objective values of the sequential and \hogwild{} algorithms, i.e. $[F(A_{\hogwildshort{}}) / F(A_{\seqalg}) - 1] \times 100\%$.
...
\textbf{Deferments:} The \occ{} algorithm defers more elements as threads are added, but less than 0.015\% are deferred with 16 threads, which has negligible effect on the runtime / speedup.
