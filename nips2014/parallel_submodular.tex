      ...
      \begin{algorithm}[H]
        ...
        \ParForAll{$p \in \set{1, \ldots, P}$}{
          \While{$\exists$ element to process}{
            ...
            $(\hat{A}_e, \tilde{A}_e, \hat{B}_e, \tilde{B}_e)$ = getSnapshot($i$)\;
            Draw $u_e\sim Unif(0,1)$\;
            res($i$) = $\Gamma_e(\hat{A}_e, \tilde{A}_e, \hat{B}_e, \tilde{B}_e, u_e)$\;
            validate($i$, res($i$), $u_e$, $\hat{A}_e$, $\tilde{A}_e$, $\hat{B}_e$, $\tilde{B}_e$)\;
          }
        }
      \end{algorithm}
