...
We develop two parallel algorithms for the maximization of non-monotone submodular functions that operate at different points along the coordination versus guarantees tradeoff curve.
...
\item We prove that \occ{} produces an outcome equivalent to the sequential double greedy algorithm, thus preserving the tight expected 1/2-approximation guarantee.
...
While those algorithms confer strong guarantees, their design is inherently serial, limiting their usability in Big Data problems.
...
This allows us to conclude that \occ{} produces a serializable result.
...
On the other hand, \hogwild{} focuses on speed, resulting in faster runtime, but delivers an objective value $F(A_{\hogwildshort{}})$ that is $20\%$ of $F(A_{\seqalgshort{}})$.
